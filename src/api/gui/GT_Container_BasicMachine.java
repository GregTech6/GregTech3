...
public class GT_Container_BasicMachine extends GT_ContainerMetaTile_Machine {
    ...
    public void detectAndSendChanges() {
    	...
    	mProgressBar = Math.max(0, Math.min(20, (((GT_MetaTileEntity_BasicMachine)mTileEntity.getMetaTileEntity()).mProgresstime>0?1:0) + (((GT_MetaTileEntity_BasicMachine)mTileEntity.getMetaTileEntity()).mProgresstime * 20) / (((GT_MetaTileEntity_BasicMachine)mTileEntity.getMetaTileEntity()).mMaxProgresstime<1?1:((GT_MetaTileEntity_BasicMachine)mTileEntity.getMetaTileEntity()).mMaxProgresstime)));
    	
        Iterator var2 = this.crafters.iterator();
        while (var2.hasNext()) {
            ICrafting var1 = (ICrafting)var2.next();
            var1.sendProgressBarUpdate(this, 100, mProgressBar);
    ...
    public void updateProgressBar(int par1, int par2) {
    	...
    	switch (par1) {
    	case 100: mProgressBar = Math.min(20, par2); break;
