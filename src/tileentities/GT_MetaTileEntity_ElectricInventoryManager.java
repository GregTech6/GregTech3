...
public class GT_MetaTileEntity_ElectricInventoryManager extends MetaTileEntity {
	...
	public void onPostTick() {
		if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().isServerSide() && getBaseMetaTileEntity().getEnergyStored() >= 5000 && (getBaseMetaTileEntity().hasWorkJustBeenEnabled() || getBaseMetaTileEntity().getTimer()%100 == 0 || mWorkedLastTick || getBaseMetaTileEntity().hasInventoryBeenModified())) {
	...
	public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
		switch (ForgeDirection.getOrientation(aSide)) {
		case DOWN : return 113 + (aRedstone?8:0);
		case UP   : return 112 + (aRedstone?8:0);
		case NORTH: return 116 + (aRedstone?8:0);
		case SOUTH: return 213 + (aRedstone?8:0);
		case WEST : return 212 + (aRedstone?8:0);
		case EAST : return 117 + (aRedstone?8:0);
		}
		return 0;
	}
