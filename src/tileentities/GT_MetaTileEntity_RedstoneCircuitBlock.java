...
public class GT_MetaTileEntity_RedstoneCircuitBlock extends MetaTileEntity implements IRedstoneCircuitBlock {
    ...
    private void resetRedstone() {
    	getBaseMetaTileEntity().setOutputRedstoneSignal((byte)0, (byte)0);
    	getBaseMetaTileEntity().setOutputRedstoneSignal((byte)1, (byte)0);
    	getBaseMetaTileEntity().setOutputRedstoneSignal((byte)2, (byte)0);
    	getBaseMetaTileEntity().setOutputRedstoneSignal((byte)3, (byte)0);
    	getBaseMetaTileEntity().setOutputRedstoneSignal((byte)4, (byte)0);
    	getBaseMetaTileEntity().setOutputRedstoneSignal((byte)5, (byte)0);
    }
    ...
    public void onPreTick() {
	    ...
	    if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().isServerSide()) {
		    ...
		    if (getBaseMetaTileEntity().getEnergyStored() > 400) {
	...
	public boolean setRedstone(byte aStrength, byte aSide) {
		if (getOutputRedstone(aSide) != aStrength) {
			if (getBaseMetaTileEntity().decreaseStoredEnergyUnits(1, false)) {
				getBaseMetaTileEntity().setOutputRedstoneSignal(aSide, aStrength);
				getBaseMetaTileEntity().setErrorDisplayID(0);
				return true;
			} else {
				getBaseMetaTileEntity().setErrorDisplayID(1);
