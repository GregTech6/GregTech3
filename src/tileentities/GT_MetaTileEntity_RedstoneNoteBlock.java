...
public class GT_MetaTileEntity_RedstoneNoteBlock extends MetaTileEntity {
    ...
    public void doSound(int aIndex, double aX, double aY, double aZ) {
        ...
        default: tStack = new ItemStack(Block.blockSteel, 1); break;
        }
        tStack.stackSize = 1+(int)(aIndex * 1.714);
        GT_Mod.gregtechproxy.doSound(tStack, getBaseMetaTileEntity().getWorld(), aX, aY, aZ);
        if (getBaseMetaTileEntity().isClientSide()) getBaseMetaTileEntity().getWorld().spawnParticle("note", aX, aY+0.7, aZ, aIndex/14.0, 0.0, 0.0);
	}
