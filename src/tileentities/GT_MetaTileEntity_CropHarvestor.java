...
public class GT_MetaTileEntity_CropHarvestor extends GT_MetaTileEntity_ElectricBufferSmall {
	...
	public void onPostTick() {
		if (getBaseMetaTileEntity().isServerSide() && getBaseMetaTileEntity().isAllowedToWork() && (getBaseMetaTileEntity().hasWorkJustBeenEnabled() || getBaseMetaTileEntity().getTimer()%20 == 0)) {
			...
			if (getBaseMetaTileEntity().getEnergyStored() >= 100*tOverclockerFactor) {
				for (int i = 0, j = (int)Math.pow(2, getBaseMetaTileEntity().getOverclockerUpgradeCount()); i < j; i++) {
					...
					if (mInventory[0] == null && getBaseMetaTileEntity().getEnergyStored() >= 64 * tOverclockerFactor && (mInventory[0] = GT_Utility.suckOneItemStackAt(getBaseMetaTileEntity().getWorld(),
							getBaseMetaTileEntity().getXCoord()+ForgeDirection.getOrientation(getBaseMetaTileEntity().getFrontFacing()).offsetX*(1+i),
							getBaseMetaTileEntity().getYCoord()+ForgeDirection.getOrientation(getBaseMetaTileEntity().getFrontFacing()).offsetY*(1+i),
							getBaseMetaTileEntity().getZCoord()+ForgeDirection.getOrientation(getBaseMetaTileEntity().getFrontFacing()).offsetZ*(1+i), 1, 1, 1)) != null) {
