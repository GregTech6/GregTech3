...
public class GT_MetaTileEntity_Barrel extends MetaTileEntity {
	...
	@Override public boolean ownerWrench()							{return mItemCount > 0;}
	...
	@Override public void onRightclick(EntityPlayer aPlayer) {
	    ...
	    if (tPlayerItem == null) {
		    if (mItemID > 0) {
	    		...
	    		aPlayer.sendChatToPlayer((mItemCount + (mInventory[0]==null?0:mInventory[0].stackSize)+(mInventory[1]==null?0:mInventory[1].stackSize)+(mInventory[2]==null?0:mInventory[2].stackSize)) + " of " + new ItemStack(mItemID, 1, mItemMeta).getDisplayName());
		    }
	    } else {
		    if (isDigitalChest && GT_OreDictUnificator.isItemStackInstanceOf(tPlayerItem, "craftingQuantumChestUpgrade", false)) {
		    	if (tPlayerItem.stackSize > 0 || aPlayer.capabilities.isCreativeMode) {
		    		...
		    		getBaseMetaTileEntity().setInitialValuesAsNBT(tNBT, 49);
		    	}
		    	return;
		    }
