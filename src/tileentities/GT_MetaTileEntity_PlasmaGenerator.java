...
public class GT_MetaTileEntity_PlasmaGenerator extends GT_MetaTileEntity_BasicTank {
	...
	@Override public int maxEUOutput()								{return 2048;}
    	...
    	if (getBaseMetaTileEntity().isServerSide() && getBaseMetaTileEntity().getTimer()%10==0) {
    		...
    		if (getBaseMetaTileEntity().isAllowedToWork() && mInventory[0] != null && getBaseMetaTileEntity().getEnergyStored() <= 100) {
    			for (int i = 0; i < GT_Recipe.sPlasmaFuels.size(); i++) {
    				if (mInventory[0].isItemEqual(GT_Recipe.sPlasmaFuels.get(i).mInput1)) {
    						...
    						if (GT_Recipe.sPlasmaFuels.get(i).mOutput1 != null) {
    							if (mInventory[1] == null) {
    								if (getBaseMetaTileEntity().increaseStoredEnergyUnits(GT_Recipe.sPlasmaFuels.get(i).mStartEU*1000, true)) {
    									...
    									mInventory[1] = GT_Recipe.sPlasmaFuels.get(i).mOutput1.copy();
    								}
    							} else if (mInventory[1].isItemEqual(GT_Recipe.sPlasmaFuels.get(i).mOutput1) && mInventory[1].stackSize + GT_Recipe.sPlasmaFuels.get(i).mOutput1.stackSize <= mInventory[1].getMaxStackSize()) {
    								if (getBaseMetaTileEntity().increaseStoredEnergyUnits(GT_Recipe.sPlasmaFuels.get(i).mStartEU*1000, true)) {
        								getBaseMetaTileEntity().decrStackSize(0, 1);
    ...
    public int getFuelValue(LiquidStack aLiquid) {
    	...
    	LiquidStack tLiquid;
    	for (int i = 0; i < GT_Recipe.sPlasmaFuels.size(); i++) {
    		if ((tLiquid = LiquidContainerRegistry.getLiquidForFilledItem(GT_Recipe.sPlasmaFuels.get(i).mInput1)) != null) if (aLiquid.isLiquidEqual(tLiquid)) return GT_Recipe.sPlasmaFuels.get(i).mStartEU;
        }
    	return 0;
    }
